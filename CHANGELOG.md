...
- **A worked platform integration,
  [`examples/platform-demo/`](https://github.com/OliverD25/crypto-processing-api/blob/main/examples/platform-demo/app.py).**
  ...
  before it was rendered — and its `/platform-webhook` handler is numbered 1:1
  with the five-step contract in `docs/integrating`. It runs as an opt-in
  compose profile on the regtest stack, and with the profile off nothing about
  that stack changes.
- **The example proves itself every night.** `scripts/dev/example_loop.py`
  ...
  re-read.
- **Two client libraries: `crypto-processing-client` for Python and
  `@oliverd25/crypto-processing-client` for TypeScript.** The request layer of
  ...
  the two things codegen cannot: an `Idempotency-Key` minted per logical call
  ...
  [`sdks/README.md`](https://github.com/OliverD25/crypto-processing-api/blob/main/sdks/README.md) for the two registry accounts that have to
  exist first.
- **Cross-language signature vectors,
  [`sdks/signature-vectors.json`](https://github.com/OliverD25/crypto-processing-api/blob/main/sdks/signature-vectors.json).** Generated by
  ...
  satisfied by a verifier that returns true unconditionally.
...
move on their own. `docs/operating/runbook-usdt-withdrawals.md` has the query to find
...
- USDT deposit attribution is heuristic, because the plugin reuses pool
  addresses. See `docs/operating/runbook-usdt-attribution.md`.
